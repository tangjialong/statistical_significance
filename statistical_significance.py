# -*- coding:utf-8 -*-
# Created by Jialong Tang on 2021-05-12

# Reference:
# 	Dror et al. The Hitchhikers Guide to Testing Statistical Significance in Natural Language Processing. In Proceedings of ACL'2018.
...
def evaluate(pred, gold, labelspace, measure=None):
	pred_count = np.zeros(len(labelspace), dtype='int32')
	gold_count = np.zeros(len(labelspace), dtype='int32')
	hit_count = np.zeros(len(labelspace), dtype='int32')

	assert len(pred) == len(gold)
	n_test = len(gold)
	error_cases = {}
	for i in range(n_test):
		y_p = labelspace.index(pred[i])
		y_g = labelspace.index(gold[i])
		pred_count[y_p] += 1
		gold_count[y_g] += 1
		if y_p == y_g:
			hit_count[y_p] += 1
		else:
			error_cases[i] = [labelspace[y_p], labelspace[y_g]]
	total_hit = sum(hit_count)
	acc = float(total_hit) / n_test


	macro_f = f1_score(y_true=gold, y_pred=pred, labels=labelspace, average='macro')
	micro_f = f1_score(y_true=gold, y_pred=pred, labels=labelspace, average='micro')

	if measure == 'acc':
		return acc
	elif measure == 'macro_f':
		return macro_f
	elif measure == 'micro_f':
		return micro_f
	else:
		return 'ACC: %0.2f, MACRO_F: %0.2f, MICRO_F: %0.2f\nERROR CASES" %s\n' %(acc, macro_f, micro_f, error_cases)
...
def pValue(algorithm1, algorithm2, golden, labelspace, measure='acc', samplelen=None, sampletime=1000, name='t-test', alpha=0.05):
	'''
		Inputs:
			algorithm1:	A list of algorithm1's outputs.
			algorithm2:	A list of algorithm2's outputs.
			golden: A list of golden references.
			labelspace: Labelspace of outputs.
			measure: The evaluation measure: [acc, macro_f, micro_f]
			samplelen: Length of data sampling.
			sampletime: Time of data sampling.
			name: Possible statistical tests: [Shapiro-Wilk, Anderson-Darling, Kolmogorov-Smirnov, t-test, Wilcoxon, McNemar, Permutation, Bootstrap]
			alpha: The desirable significance level. 
		Output:
	'''
	assert len(algorithm1) == len(golden)
	assert len(algorithm2) == len(golden)
	n_test = len(golden)

	if samplelen is None:
		samplelen = n_test

	data_A = []
	data_B = []
	# sample the results of applying each algorithm (A and B) on the data
	for i in range(sampletime):
		...
		for j in range(samplelen):
			...
			tmp1.append(algorithm1[n])
			tmp2.append(algorithm2[n])
			tmp_golden.append(golden[n])
		data_A.append(evaluate(tmp1, tmp_golden, labelspace, measure))
		data_B.append(evaluate(tmp2, tmp_golden, labelspace, measure))

	print (evaluate(algorithm1, golden, labelspace))
	print (evaluate(algorithm2, golden, labelspace))
...
if __name__ == '__main__':
	# test this module
	labelspace = ['Positive', 'Negative', 'Natural']

	algorithm1 = [random.choice(labelspace) for i in range(100)]
	algorithm2 = [random.choice(labelspace) for i in range(100)]
	golden = [random.choice(labelspace) for i in range(100)]

	pValue(algorithm1, algorithm2, golden, labelspace)